...
def structure_plot(save_dir,
    ...
    if not len(npreds.index) == len(npreds.columns):
        raise ValueError("Number of pops does not \
    ...
    sn.set_style('ticks')
    f = plt.figure()
    splot = npreds.plot(kind='bar', stacked=True,
                        colormap=ListedColormap(
                            sn.color_palette(col_scheme,
                                             num_classes)
                        ), 
                        figsize=(12,6),
                        grid=None)
    ...
    plt.savefig(save_dir+"/structure_plot.png",
                bbox_inches = "tight")