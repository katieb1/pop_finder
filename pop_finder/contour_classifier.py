...
def contour_classifier(
    ...
    **kwargs,
    ...
    return_df : boolean
        If true, saves the results in a csv file in the save_dir folder (Default=True).
    save_dir : string
        Folder to save results. Folder should already be in directory. If using results
        from multiple iterations, only include prefix (e.g. 'out' for 'out1', 'out2',
        'out3'; Default='out').
    multi_iter : int
        How many times to run locator to get predictions. If the sample size is small,
        it is better to run multiple iterations (Default=1).
    ...
    if run_locator==True:
        ...
        for i in range(1, multi_iter+1):
            for j in range(nboots):
                out_list.append(save_dir+str(i)+"/loc_boot"+str(j)+"_predlocs.txt")
        ...
        if multi_iter == 1:
            ...
            if os.path.isdir(save_dir) is True:
                ...
                if os.path.exists(save_dir+"/loc_boot"+str(nboots+1)+"_predlocs.txt"):
                    raise ValueError("Number of bootstraps in output directory does not\
                    match nboots specified")

                out_list = [save_dir+"/loc_boot"+str(i)+"_predlocs.txt" for i in range(nboots)]
                
                if (sum(1 for line in open(out_file_test[0])) != 
                    sum(1 for line in open(out_file_test[1]))):
                ...
                # Check to make sure right number of boots selected
                if os.path.exists(save_dir+"1/loc_boot"+str(nboots+1)+"_predlocs.txt"):
                    raise ValueError("Number of bootstraps in output directory does not\
                    match nboots specified")

                out_list = [save_dir+"1/loc_boot"+str(i)+"_predlocs.txt" for i in range(nboots)]
    ...
    if isinstance(sample_data, pd.DataFrame) is not True:
        true_dat = pd.read_csv(sample_data,
                               sep="\t")
    else:
        true_dat = sample_data
    
    if len(true_dat.columns) != 4:
        raise ValueError("sample_data file is in wrong format - make sure\
...
def kfcv(sample_data, gen_dat, n_splits=5, n_runs=5,
         return_plot=True, save_dir='kfcv', **kwargs):
    """
    ...
    **kwargs
    ...
    for i in range(n_runs):
        ...
        for train_index, test_index in skf.split(true_dat.index.values, true_dat['pop']):
            k = true_dat.copy()
            k.iloc[test_index, 0:3] = np.NaN
            k.to_csv('kfcv_tmp.csv', sep='\t', index=False)

            # k becomes the sample data that goes into locator
            class_df = contour_classifier(sample_data='kfcv_tmp.csv',
                                          run_locator=True,
                                          gen_dat=gen_dat,
                                          return_plots=False,
                                          return_df=False,
