...
def contour_classifier(
    ...
    save_dir : string
        Folder to save results. Folder should already be in directory
        (Default='out').
    """

    # Run locator
    if run_locator:
        locator_mod.locator(sample_data, gen_dat,
                            bootstrap=True, nboots=nboots)

    # Create list of predloc files for concatenation
    out_list = [
        save_dir + "/loc_boot" + str(i) + "_predlocs.txt" for i in range(
            nboots
        )
    ]
    ...
    for sample in pred_dat["sampleID"].drop_duplicates():
        ...
        cset = ax.contour(X, Y, new_z, levels=10, colors="black")
