...
def test_read():
    ...
    # Read data w/o kfcv
    x = read.read_data(infile_all, sample_data2, save_allele_counts=False)
    ...
    # Read data w/ kfcv
    y = read.read_data(infile_all, sample_data1, save_allele_counts=False,
                       kfcv=True)
...
def test_hp_tuning():
    ...
    hm_test = hp_tuning.classifierHyperModel(input_shape=2,
                                             num_classes=2)
...
def test_pop_finder():
    ...
    pop_finder.run_neural_net(infile_kfcv, infile_all, sample_data2,
                              ...
                              tune_model=False, n_splits=5, n_reps=1,
    ...
    with pytest.raises(ValueError):
        pop_finder.run_neural_net(infile_kfcv, infile_all, sample_data2,
                            save_allele_counts=False, save_weights=False,
                            patience=10, batch_size=32, max_epochs=2,
                            seed=1, train_prop=0.95, gpu_number='0',
                            tune_model=False, n_splits=5, n_reps=1,
                            save_best_mod=False, save_dir=save_path,
                            plot_history=False)
